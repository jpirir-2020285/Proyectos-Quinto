...
Delimiter $$
    ...
    End$$
Delimiter ;
call sp_AgregarProveedor('1231231','miguel','pirir','calle 13','trabajar','telefono casa:14124323','KinalAcademy');
...
drop procedure sp_EditarProveedores;
-- Editar
Delimiter $$
...
-- Agregar
Delimiter $$
	Create procedure sp_AgregarCompra ( in fechaDocumento date, 
		in descripcion varchar(60), in totalDocumento decimal(10,2))
    Begin
...
Delimiter $$
	Create procedure sp_EditarCompra (in numDocumento int, in fechDocumento date, 
		in descrip varchar(60), in toDocumento decimal(10,2))
    Begin
		Update  Compras C
			Set 
				C.fechaDocumento = fechDocumento, 
				C.descripcion = descrip, 
				C.totalDocumento = toDocumento
			Where numeroDocumento = numDocumento;
    End$$
Delimiter ;
...
Delimiter $$
    ...
    End$$
Delimiter ;
call sp_AgregarCliente('213412','Jose Miguel','Pirir Perez','Zona 16 sant','123412','josemiguelin');
...
-- call sp_AgregarCargoEmpleado('empleado','hace lo que le pidan');
...
-- Agregar
Delimiter $$
	Create procedure sp_AgregarProductos ( in descripcionProducto varchar(45), 
		in precioUnitario decimal(10,2), in precioDocena decimal(10,2), in precioMayor decimal(10,2), 
		in imagenProducto varchar(45), in existencia int, in codigoTipoProducto int, in codigoProveedor int)
    Begin
		Insert into Productos ( descripcionProducto, precioUnitario, precioDocena, precioMayor, imagenProducto, existencia, codigoTipoProducto, codigoProveedor)
			values ( descripcionProducto, precioUnitario, precioDocena, precioMayor, imagenProducto, existencia, codigoTipoProducto, codigoProveedor);
    End$$
Delimiter ;
...
Delimiter $$
	Create procedure sp_EditarProductos (in codProducto varchar(15), in descripProducto varchar(45), 
		in preciUnitario decimal(10,2), in preciDocena decimal(10,2), in preciMayor decimal(10,2), 
		in imageProducto varchar(45), in exist int)
    Begin
		Update  Productos P
			Set 
				P.descripcionProducto = descripProducto, 
				P.precioUnitario = preciUnitario, 
				P.precioDocena = preciDocena, 
				P.precioMayor = preciMayor, 
				P.imagenProducto = imageProducto, 
				P.existencia = exist
			Where codigoProducto = codProducto;
    End$$
Delimiter ;
...
-- Agregar
Delimiter $$
	Create procedure sp_AgregarEmpleados ( in nombresEmpleado varchar(50), 
    ...
    Begin
		Insert into Empleados ( nombresEmpleado, apellidosEmpleado, sueldo, direccionEmpleado, turno, codigoCargoEmpleado)
			values ( nombresEmpleado, apellidosEmpleado, sueldo, direccionEmpleado, turno, codigoCargoEmpleado);
    End$$
Delimiter ;
...
-- Editar
Delimiter $$
	Create procedure sp_EditarEmpleados (in codEmpleado int, in nomEmpleado varchar(50), 
...
-- Agregar
Delimiter $$
	Create procedure sp_AgregarFactura (in estado varchar(50), 
		in totalFactura decimal(10,2), in fechaFactura date, in codigoCliente int, 
        in codigoEmpleado int)
    Begin
		Insert into Factura ( estado, totalFactura, fechaFactura, codigoCliente, codigoEmpleado)
			values ( estado, totalFactura, fechaFactura, codigoCliente, codigoEmpleado);
    End$$
Delimiter ;
...
Delimiter $$
    ...
    Begin
		Update Factura F
			Set 
				estado = est, 
                totalFactura = totFactura, 
                fechaFactura = fechFactura
			Where numeroFactura = numFactura;
    End$$
Delimiter ;
...
-- Agregar
Delimiter $$
	Create procedure sp_AgregarDetalleFactura (in precioUnitario decimal(10,2), in cantidad int, 
        in numeroFactura int, in codigoProducto varchar(15))
    Begin
		Insert into DetalleFactura ( precioUnitario, cantidad, numeroFactura, codigoProducto)
			values ( precioUnitario, cantidad, numeroFactura, codigoProducto);
    End$$
Delimiter ;
...
-- Editar
Delimiter $$
	Create procedure sp_EditarDetalleFactura (in codDetalleFactura int, 
		in preciUnitario decimal(10,2), in cant int)
    Begin
...
Delimiter $$
	Create trigger tr_TotalDocumento_After_Insert
        ...
        begin
			update Compras
				set
					Compras.totalCompra = Compras.totalcompra + (new.costoUnitario*new.cantidad)
            ...
            update Productos P
				set
                    ...
                    P.existencias = P.existencias + new.Cantidad
